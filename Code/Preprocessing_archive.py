...
for col in cols1:
    ...
    if len_answers <= 7:
        replace = [6, 7]
        df1_replaced[col] = df1[col].replace(replace,[np.NaN,np.NaN])
...
#%%
"""
Round 17 - drop all except K2Q34A
Then simple imputer
"""
df_target = df1.loc[df1[target].isin([0,1])]
df_target.info()

df = df_target.drop([x for x in df_target.columns if x not in ['K2Q34A']], axis = 1)
Y = pd.DataFrame(df_target['K2Q31A'])
#%%
"""
Correlation Matrices
"""
#df1_corr = df1.corr()
df1_corr = df1_conduct.corr()
...
# Get strongest corrs
keep = []
for  row in list(df1_corr_target.index.values):
    if (df1_corr_target.at[row,'Corr_subset'] >.1) | (df1_corr_target.at[row, 'Corr_subset'] < -.1):
        keep.append(row)
...
#df1_corr_strong_T.sort_values(by = 'Corr_subset', inplace = True, ascending = False)
df1_corr_strong.to_csv('Target_corr_strong_no_conduct_in_any.csv')
...
#clean_targeted_w_conduct = clean_targeted.loc[clean_targeted[ 'K2Q34A'].isin([1])]
df1_conduct = df1_replaced.loc[df1_replaced[ 'K2Q34A'].isin([0,6])]

# keep with and without conduct for no ADHD, keep ADHD only with conduct
#df1_conduct = df1[(df1[ 'K2Q34A'].isin([1])) | (df1['K2Q31A'] == 0)]
#df[df['Age'] >= 25] 
df1_conduct.info()

#%%
# Split into x and Y dfs
x = df1_conduct.iloc[:,1:]
Y = pd.DataFrame()
Y[target] = df1_conduct[target]
...
"""
15th Test - K-means on data with K2Q31A = 1
"""
#clean_targeted_only_ADHD = df1.loc[df1[ 'K2Q31A'].isin([1])]
clean_targeted_only_ADHD = df1_no_conduct.loc[df1_no_conduct[ 'K2Q31A'].isin([1])]
clean_targeted_only_ADHD.info()
clean_targeted_only_ADHD.drop(['K2Q31A', 'IDNUMR', 'NSCHWT'], axis = 1, inplace = True)
 
#%%
"""
# used simple imputer below 
x = clean_targeted_only_ADHD.copy() 

#%%
xtrans.to_csv('keep_15_no_conduct_only_ADHD_code_2.csv', index=False)
# normalize data
from sklearn import preprocessing
clean_targeted_only_ADHD = xtrans 
x_have_vals = preprocessing.normalize(clean_targeted_only_ADHD)
clean_targeted_only_ADHD = pd.DataFrame(data = x_have_vals, columns = list(xtrans))

clean_targeted_only_ADHD.to_csv('keep_15_NORM_no_conduct_only_ADHD_code_2.csv', index = False)
"""
#%%
x = df.copy()
...
xtrans = pd.DataFrame(data = xtrans_vals, columns = list(x_have_vals))
#%%
xtrans.to_csv('keep_18_w_conduct_only_K2Q34A.csv', index = False)
Y.to_csv('Y18.csv', index = False)
...
xtrans.drop(['IDNUMR', 'NSCHWT'], axis = 1, inplace = True)

        
#%%
# Move up with rest of codes_keep
dropped = []
...
#%%
xtrans.to_csv('x_keep_16_no_conduct_code_12.csv', index = False)
Y.to_csv('Y16.csv', index = False)
...
#remove_outliers
threshold = 1
df = df1[np.abs(df1['NSCHWT'] - df1['NSCHWT'].mean()) / df1['NSCHWT'].std() >= threshold]
idnums_outliers = df[df['IDNUMR'] >= 2300]